...
class Meter(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.amountminvar = tk.IntVar(value=amountmin) # dodato
    ...
    def _meter_value(self) -> int:
        ...
        progress meter."""
        value = int(
            ((self["amountused"]-self["amountmin"]) / (self["amounttotal"]-self["amountmin"])) * self._arcrange
            + self._arcoffset
        )
        return value
    ...
    def _on_dial_interact(self, e: tk.Event):
        ...
        amountused = (amountmin + ((amounttotal-amountmin) / self._arcrange * factor))

        # calculate amount used given stepsize
        if amountused > amountmin + self._stepsize/2:
            amountused = amountused // self._stepsize * self._stepsize + self._stepsize
        else:
            amountused = amountmin
        # if the number is the name, then do not redraw
        if lastused == amountused:
            return
        # set the amount used variable
        if amountused <= amountmin: # dodato
            self.amountusedvar.set(amountmin) # dodato
        elif amountused >= amounttotal: # dodato
            self.amountusedvar.set(amounttotal) # dodato
        else:
            self.amountusedvar.set(int(amountused))
